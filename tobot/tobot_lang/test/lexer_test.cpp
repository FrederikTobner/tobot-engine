...
std::vector<TokenizationRule<TokenType>> rules = {{NUMBER, "[0-9]+"},  {PLUS, "\\+"}, {MINUS, "-"},
...
TEST(Lexer, CanTokenizeMultipleTokens) {
    Lexer<TokenType> lexer(rules);
    std::vector<Token<TokenType>> tokens = lexer.tokenize("- 123");
...
TEST(Lexer, CanTokenizeComplexExpression) {
    Lexer<TokenType> lexer(rules);
    std::vector<Token<TokenType>> tokens = lexer.tokenize("1 + 2 * 3 - 4 / 5");
...
TEST(Lexer, CanHandleLineBreaks) {
    Lexer<TokenType> lexer(rules);
    std::vector<Token<TokenType>> tokens = lexer.tokenize("1 \n2");
    ASSERT_EQ(tokens.size(), 2);
    ASSERT_EQ(tokens[0].getType(), NUMBER);
    ASSERT_EQ(tokens[0].getLexeme(), "1");
    ASSERT_EQ(tokens[0].getLine(), 1);
